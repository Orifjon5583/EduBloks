...
import { Search, Plus, Key, Trash2, UserPlus } from 'lucide-react';
import api from '../../api';
export default function Students() {
  ...
  return <div><div className="flex justify-between items-center mb-4"><h1 className="text-2xl font-bold dark:text-white">Talabalar</h1><button onClick={()=>setShow(true)} className="btn-primary flex items-center gap-2"><UserPlus className="w-4 h-4"/>Yangi</button></div>
    <div className="relative max-w-sm mb-4"><Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"/><input value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}} placeholder="Ism, familiya yoki login..." className="input-field pl-10"/></div>
